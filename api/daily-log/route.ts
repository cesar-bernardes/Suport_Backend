...
function validate(input: ReturnType<typeof fields>) {
  if (!validDate(input.workDate) || !validTime(input.time)) return "Informe uma data e um horário válidos.";
  if (input.activity.length < 3 || input.activity.length > 160) return "A atividade deve ter entre 3 e 160 caracteres.";
  if (input.observations.length > 1500 || input.actions.length > 1500) return "Observações e ações devem ter no máximo 1.500 caracteres.";
  return null;
}
...
export async function POST(request: Request) {
  ...
  if (!(await validManagedUserId(input.assigneeId))) return apiError(422, "Responsável inválido.");
  if (user.role === "suporte" && input.assigneeId !== user.id) return apiError(403, "Você só pode preencher seu próprio diário.");
...
export async function PATCH(request: Request) {
  ...
  if (!current) return apiError(404, "Atividade não encontrada.");
  if (!canAccess(user, current)) return apiError(403, "Você não pode alterar esta atividade.");
  const input = fields(body);
  const issue = validate(input);
  if (issue) return apiError(422, issue);
  if (!(await validManagedUserId(input.assigneeId))) return apiError(422, "Responsável inválido.");
  if (user.role === "suporte" && input.assigneeId !== user.id) return apiError(403, "Você não pode reatribuir esta atividade.");
  const entry = await updateDailyLog({ ...current, ...input });
  return entry ? jsonResponse({ entry }) : apiError(404, "Atividade não encontrada.");
}

export async function DELETE(request: Request) {
  const user = await sessionUser(request);
  if (!user) return apiError(401, "Sessão inválida ou expirada.");
  if (user.role !== "administrador") return apiError(403, "Somente Administradores podem remover registros do diário.");
  if (!sameOriginMutation(request)) return apiError(403, "Origem da requisição não autorizada.");
  const id = new URL(request.url).searchParams.get("id")?.trim() || "";
  if (!id || !(await getDailyLog(id))) return apiError(404, "Atividade não encontrada.");
  if (!(await softDeleteDailyLog(id, user.id))) return apiError(422, "Não foi possível remover a atividade.");
